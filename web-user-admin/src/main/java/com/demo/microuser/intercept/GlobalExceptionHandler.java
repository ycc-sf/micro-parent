...
public class GlobalExceptionHandler {
	...
	public RestResponse<Nullable> exceptionGet(HttpServletRequest req,HttpServletResponse response ,Exception e) {
		if (e instanceof ApplicationException) {
		    ApplicationException be = (ApplicationException) e;
			if(WebErrorCode.CUSTOM.equals(be.getErrorCode())){
...
//			return new RestResponse<Nullable>(304, "没有权限访问");
		...
		LOGGER.error("【系统异常】{}", e);
		return  new RestResponse<Nullable>(WebErrorCode.UNKOWN.getCode(),WebErrorCode.UNKOWN.getDesc());
	}
